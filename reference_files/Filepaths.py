...
class SearchFlags:
    ...
    Filetypes = namedtuple(
        ...
        defaults=[pathlib.Path.cwd(), True, True, 'txt'],  # no dot on the extension
        rename=True,
    )
...
HardcodedExcludes = [".git", ".venv", "__pycache__", ".vscode", ".idea", ".run"]
# TODO: also exclude any path that isn't under the user's home directory
# currently, the key is expected to be an all-uppercase match for the file extension (associated with that path)
subfolders = {
    #"CSV": ["source", "saved"],
    #"JSON": [],
    "TXT": ["asdf"],
    "TESTFILES": ["placeholder", "examples"],
...
DefaultFiletypes = SearchFlags.Filetypes(pathlib.Path.cwd(), True, True, 'txt')
...
# TODO: try using pattern matching to find files
...
if __name__ == "__main__":
    ...
    globresults = list(itertool.collapse([
        ...
        GlobSearch(opt=SearchFlags.Filetypes(derivepath=True, extension="txt")),
